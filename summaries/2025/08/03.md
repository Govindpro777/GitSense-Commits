...
## 10:32:56 PM
The `Signup.jsx` component underwent significant changes on August 3rd, 2025, at 9:33:38 PM.  The code implements a user signup form with features for both early access users (using a token) and standard users.  The form handles user input for organization name, email, phone number, name, password, and a 4-digit OTP.  Extensive validation is performed on the inputs, checking for minimum lengths, password complexity, and email domain restrictions (disallowing public email domains for standard users).

The component uses several React hooks (`useState`, `useRef`, `useSearchParams`, `useRouter`) and interacts with a `userService` for fetching early access information and performing signup.  Early access information is fetched using a token obtained from URL parameters, and the fetched data populates the form fields.  There's a countdown timer implemented for OTP verification.  Local storage is used to temporarily store user information, then cleaned up after login attempts.  Error handling and notifications are integrated using a custom `useNotification` hook.  The signup process itself is asynchronous (`handleSignup`).  The code also includes input focus management for the OTP fields using refs and event handling to streamline OTP entry.  The component clears local storage on mount and removes any pre-existing user or userInfo data. Finally, successful signup redirects the user to '/summary'.


## 11:33:01 PM
The `Signup.jsx` component, last updated on August 3rd, 2025 at 11:02:09 PM, handles user signup functionality.  It uses several React hooks (`useEffect`, `useState`, `useRouter`, `useSearchParams`) and interacts with a `userService` for API calls.

The component manages user input for organization name, email, phone number, password, and a 4-digit OTP (One-Time Password).  Input validation is performed before submitting the signup request.  Public email domains are disallowed unless early access is granted via a token passed in the URL's search parameters.

Significant features include:

* **Early Access:**  If a token (`id`) is provided in the URL, the user is considered for early access, and their information is pre-populated from an API call to `/EARLY_ACCESS_INFO`.
* **OTP Verification:** Upon initial signup, an OTP is sent to the user. A countdown timer is implemented for OTP expiry.  Incorrect OTPs trigger an error and input field clearing.
* **Signup Handling:** The `handleSignup` function sends a request to `/SOCIAL_SIGNUP` with the collected user data. It handles various response statuses, such as `Ok`, `ActiveUser`, `AlreadySocialUser`, `InActiveUser`, `AlreadySignUp`, `BadRequest`, `OTP`, `InvalidOTP`, and `ExpireOTP`, redirecting the user to appropriate pages (`/summary`, `/login`) and displaying notifications accordingly.
* **Local Storage:** User data is stored in local storage after successful signup, and then removed on component mount.  This storage is used for information fetched from `/USERINFO`.

The code extensively uses error handling and displays user-friendly notifications using a custom `useNotification` hook.  Several `useEffect` hooks manage side effects like fetching early access information, cleaning up timers, and clearing local storage.  The OTP input is managed using an array of states and controlled input refs to automatically advance focus to the next input field.  A `onPaste` function handles pasting of OTP values, limiting it to a maximum of 4 characters.  Finally password validation enforces a minimum length, including uppercase letters, numbers, and special characters.
