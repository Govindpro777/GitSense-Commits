...
## 10:11:36 PM
The log shows updates to authentication-related code on both the backend (restaurant-backend) and frontend (restaurant-frontend) of a Zomato-like application.

On **9/1/2025, 9:20:32 PM**, `authController.js` (backend) was modified.  The key change in the `register` function was the addition of a `role` field to the request body and the ability to set a default role ('user') if not provided.  The `login` function remained largely unchanged, but now includes the user's name in the response.  Both functions handle potential errors and respond with appropriate status codes.

On **9/1/2025, 9:25:34 PM**, `auth.js` (frontend) was updated.  This file contains functions for handling user login and logout.  The `login` function uses `axios` to make a POST request to the backend API. Upon successful login, it stores the received token and user information (email, ID, role, modules) in the browser's local storage. The `logout` function clears this local storage.  The API URL is dynamically determined, potentially using environment variables.
