...
## 5:53:53 PM
The code changes primarily focus on implementing premium content gating and enhancing loading states across several dashboard and journalist profile components. A consistent pattern of blurring content and providing upgrade prompts for "Freebies" users is evident.

**File-Specific Updates:**

*   **`c:\Users\91965\Desktop\ScribeIntern\SKRIBE NEW FRONT_END\skribefrontend\src\components\JP\BehavioralPattern.jsx` (Timestamp: 11/4/2025, 1:48:48 PM):**
    *   **Data Fetching Refinement:** The component now uses a single `JOURNALISTBRAND` API endpoint to fetch `spokespersonData`, `topicsData`, and `brand` data, consolidating previous separate or commented-out API calls.
    *   **Premium Content Locking:** Extensive conditional rendering has been introduced. If the user's `roleType` includes "Freebies," charts and data lists (Spokespeople, Topics) are replaced with blurred sections, `react-loading-skeleton` placeholders, an "Unlock More with Premium" message, and an "Upgrade to View" button that triggers a `showPopup()` function. Otherwise, the actual `ColoumnChart` and data lists are rendered.
    *   **UI Enhancements:** Skeleton loaders are used for better visual feedback during data loading.

*   **`c:\Users\91965\Desktop\ScribeIntern\SKRIBE NEW FRONT_END\skribefrontend\src\components\JP\LineChart.jsx` (Timestamp: 11/4/2025, 1:49:02 PM):**
    *   **Data Fetching:** Fetches journalist frequency count data using the `JOURFREQCOUNT` endpoint, processing it to set chart `categories` (week start dates) and `series` (article counts).
    *   **Premium Content Locking:** Similar to `BehavioralPattern.jsx`, if the user has a "Freebies" `roleType`, the line chart area is replaced with a blurred section, `react-loading-skeleton` content, an upgrade message, and a link to `showPopup()`. The actual `Chart` component is rendered only for premium users.
    *   **Chart Configuration:** Standard ApexCharts options for a line chart displaying frequency data.

*   **`c:\Users\91965\Desktop\ScribeIntern\SKRIBE NEW FRONT_END\skribefrontend\src\components\Dashboard\Skribe365.jsx` (Timestamp: 11/4/2025, 2:04:50 PM and 11/4/2025, 2:05:31 PM):**
    *   **Minor Refinement:** The two timestamps are very close, indicating a quick save or a minor refactor. The primary change between these two versions is the explicit wrapping of the premium content blocking UI (`blur-sm` and upgrade message) within a conditional `roleType?.includes(Freebies)` block, formalizing its display for freebie users.
    *   **Data Fetching Control:** Data for "Skribe 365" is now only fetched if the user's `roleType` does *not* include "Freebies."
    *   **Premium Content Locking:** For "Freebies" users, the entire content area is blurred and replaced with skeletons, an "Unlock More with Premium" message, and an "Upgrade to View" button linked to `showPopup()`.
    *   **Content Display:** For non-Freebies users, articles are fetched, grouped by `primaryBeat`, and displayed with `outletname`, `createdDate`, and a truncated `description` using a `ReadMore` helper component (which sanitizes HTML content with `DOMPurify`).
    *   **Navigation:** A "View All" link is provided to navigate to `/view-skribe365`.
    *   **Loading States:** Skeletons are used while articles are loading for premium users.

**Patterns and Recurring Elements:**

*   **Premium Content Gating Strategy:** A consistent and pervasive pattern across all updated files is the implementation of premium content gating. This involves:
    *   Checking the user's `roleType` (obtained via `tokenService.getLocalRole()`) for the presence of "Freebies."
    *   Displaying blurred sections with `react-loading-skeleton` placeholders.
    *   Presenting a standardized "Unlock More with Premium" message.
    *   Including an "Upgrade to View" button which triggers a `showPopup()` function, likely originating from a global popup context (`usePopup`).
*   **Loading Indicators:** `react-loading-skeleton` is extensively used to provide visual feedback to users while data is being fetched or processed, improving the perceived performance.
*   **Service Layer Interaction:** All components utilize `userService` to interact with backend APIs, with API endpoints defined in a `../../constants` file.
*   **Component Structure:** Components are client-side Next.js components (`"use client";`) and use React hooks like `useEffect` and `useState` for managing side effects and local state.
*   **Styling:** Tailwind CSS classes (e.g., `flex`, `mx-2`, `rounded-[10px]`, `blur-sm`) are widely used for layout and styling.