...
## 10:31:41 PM
The `app.js` file, updated on `11/22/2025, 4:09:13 PM`, outlines the core Express application setup, focusing on security, performance, and API routing.

Key updates include:
*   **Enhanced CORS Configuration:** A custom CORS setup is implemented using an `allowedOrigins` array. This array explicitly permits access from `http://localhost:3000`, `http://localhost:5173`, `http://localhost:8000`, `https://app.manmitr.com`, `http://manmitr.com`, `https://api.manmitr.com`, and Render.com deployments (`https://kidmantree-backend-g2la.onrender.com`, `https://manmitr.onrender.com`). This replaces a simpler, commented-out CORS configuration.
*   **Security Middleware:** `helmet` is configured to specifically allow `cross-origin` resource policies, enabling consumption of assets like images from different origins.
*   **Rate Limiting:** `express-rate-limit` is applied to all `/api` routes, with the window and maximum requests configurable via environment variables (`RATE_LIMIT_WINDOW` and `RATE_LIMIT_MAX_REQUESTS`).
*   **Body Parsing and Compression:** The application uses `compression()`, `express.json()`, and `express.urlencoded()` for request body parsing, each configured with a `10mb` size limit.
*   **Logging:** `morgan` is integrated for HTTP request logging, directing output to a custom `logger` utility at the `info` level.
*   **Static File Serving:** An endpoint `/uploads` is configured to serve static files from the local `uploads/` directory.
*   **API Routing and Documentation:** API routes are mounted under `/api`, and Swagger documentation is set up using `setupSwagger(app)`.
*   **Health Check Endpoint:** A new `/health` GET endpoint is added, providing application status, timestamp, uptime, and environment information.
*   **Error Handling:** Custom `notFoundHandler` and `errorHandler` middlewares are integrated to manage application errors and 404 responses.

The changes collectively indicate a focus on robust API infrastructure, incorporating security best practices, performance optimizations, and better observability through logging and a health check.

## 11:31:36 PM
The `app.js` file, updated on 11/22/2025 at 4:09:13 PM, received significant updates primarily focused on enhancing security, performance, and API management for the backend application.

Key changes include:
*   **CORS Configuration Refinement:** The CORS middleware was updated to use a dynamic `allowedOrigins` array, explicitly permitting requests from multiple specific origins including `http://localhost:3000`, `http://localhost:5173`, `https://app.manmitr.com`, `http://manmitr.com`, `http://localhost:8000`, `https://api.manmitr.com`, `https://kidmantree-backend-g2la.onrender.com`, and `https://manmitr.onrender.com`. This ensures tighter control over cross-origin requests compared to a potentially broader or commented-out configuration.
*   **Security Enhancements:**
    *   `helmet` middleware was configured with `crossOriginResourcePolicy: { policy: "cross-origin" }` to allow cross-origin resource consumption.
    *   `express-rate-limit` was implemented for the `/api` routes, using environment variables `RATE_LIMIT_WINDOW` and `RATE_LIMIT_MAX_REQUESTS` to control the number of requests, defaulting to 15 minutes and 100 requests respectively.
*   **Performance Optimization:** `compression` middleware was added to reduce the size of response bodies.
*   **Body Parsing Limits:** Both `express.json()` and `express.urlencoded()` were configured with a `limit: "10mb"`, standardizing the maximum payload size.
*   **Logging Integration:** `morgan` was set up to use a custom `logger` utility for logging HTTP requests.
*   **API Structure and Documentation:**
    *   Static file serving for `/uploads` was enabled.
    *   API routes were consolidated under `/api` using a dedicated `routes` module.
    *   Swagger documentation setup (`setupSwagger(app)`) was integrated.
*   **Health Check Endpoint:** A new `/health` endpoint was added to provide basic application status, uptime, and environment information.
*   **Centralized Error Handling:** `notFoundHandler` and `errorHandler` middlewares were consistently applied at the end of the middleware chain for comprehensive error management.

**Patterns and Recurring Elements:**
*   **Environment Variable Reliance:** The application heavily leverages `process.env` for configuration, seen in CORS origin checks, rate limiting parameters, and potentially other aspects of the application.
*   **Focus on Security:** Multiple layers of security middleware (Helmet, CORS, Rate Limiting) indicate a strong emphasis on protecting the API.
*   **Structured Middleware Application:** Middleware is applied in a logical order: security, body parsing, compression, logging, static files, routes, documentation, health check, and finally error handling.
*   **Consistent Limits:** A `10mb` limit for body parsing appears consistently for both JSON and URL-encoded payloads.